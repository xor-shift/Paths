...
struct Matrix : public Impl::MatrixExpr<T, Matrix<T, M, N>> {
    ...
    constexpr explicit Matrix(const E &expr) noexcept {
        for (size_t j = 0; j < M; j++)
            for (size_t i = 0; i < N; i++)
                impl[i + j * N] = expr.At(j, i);
    }

    constexpr explicit Matrix(flat_type &&data) noexcept { std::copy(data, data + M * N, impl); }

    constexpr explicit Matrix(const flat_type &data) noexcept { std::copy(data, data + M * N, impl); }
...
constexpr inline auto operator*(const E0 &e0, const U s) noexcept {
...
template<Concepts::MatrixExpression E0>
constexpr inline auto Reciprocal(const E0 &e0) noexcept {
...
constexpr inline auto operator+(const E0 &e0, const E1 &e1) noexcept {
...
template<Maths::Concepts::MatrixExpression E>
