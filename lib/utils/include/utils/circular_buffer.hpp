...
/// \tparam bufferSize
/// \tparam Alloc
template<typename T, std::size_t bufferSize, typename Alloc = std::allocator<T>>
class CircularBuffer { ///TODO: add support for throwing constructors
  ...
  public:
    ...
    constexpr explicit CircularBuffer() noexcept(std::is_nothrow_default_constructible_v<AllocType>)
        : allocator(AllocType()), buffer(allocator.allocate(bufferSize)) {}

    constexpr explicit CircularBuffer(const AllocType &allocator) noexcept(std::is_nothrow_default_constructible_v<AllocType>)
        : allocator(allocator), buffer(allocator.allocate(bufferSize)) {}

    constexpr explicit CircularBuffer(AllocType &&allocator) noexcept(std::is_nothrow_move_constructible_v<AllocType>)
        : allocator(allocator), buffer(allocator.allocate(bufferSize)) {}
    ...
    constexpr reference front() noexcept { return ConstCastCall(reference, front); }

    constexpr reference back() noexcept { return ConstCastCall(reference, back); }
  ...
  private:
    ...
    static constexpr std::size_t Inc(std::size_t v) { return (v + 1) % bufferSize; }

    static constexpr std::size_t Dec(std::size_t v) { return (v + bufferSize - 1) % bufferSize; }
};
