...
struct AABox {
  ...
  public:
    constexpr AABox(size_t matIndex, Point min, Point max) noexcept
      : extents(min, max), center((min + max) / 2.), matIndex(matIndex) {}
    ...
    [[nodiscard]] constexpr std::optional<Intersection> Intersect(const Ray &ray) const noexcept {
        if (const auto &[isect, dist] = Intersects<true>(ray); isect) {
            ...
            const auto p = isection.intersectionPoint - (extents.first + extents.second) * static_cast<Real>(.5);
            const auto d = (extents.first - extents.second) * static_cast<Real>(.5);
            const Real bias = 1.000001;

            isection.normal = Maths::Normalized(Point(
...
constexpr bool InBounds(const std::pair<Point, Point> &b0, Point p) {
    for (size_t i = 0; i < Point::vectorSize; i++) {
        if (b0.first[i] > p[i]) return false;
        if (b0.second[i] < p[i]) return false;
    }
    return true;
}
